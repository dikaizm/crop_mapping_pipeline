...
def assign_nodata(in_path: str, out_path: str, overwrite: bool = False) -> str:
    """
    Optimized: float64 → float32, DEFLATE+predictor=3, band-interleaved tiled 256×256, pyramids.

    Band-interleaved layout keeps same-band neighbours adjacent so predictor=3
    (IEEE 754 byte-shuffling) can exploit spatial autocorrelation across all bands.
    Profile built from scratch — never copied from raw — to avoid inheriting
    interleave='pixel' which defeats the predictor and inflates the output ~3×.
    """
    out = Path(out_path)
    ...
    try:
        with rasterio.open(in_path) as src:
            # Build profile from scratch — never copy from raw to avoid inheriting
            # interleave='pixel' which breaks tiled layout and kills compression.
            profile = {
                "driver":     "GTiff",
                "dtype":      "float32",
                "nodata":     S2_NODATA,
                ...
                "compress":   "deflate",
                "predictor":  3,           # IEEE 754 float predictor
                "interleave": "band",      # same-band neighbours adjacent → predictor works
                "tiled":      True,
                "blockxsize": 256,
                "blockysize": 256,
            }
        ...
        with rasterio.open(tmp, "r+") as dst:
            dst.build_overviews([4, 8, 16, 32], Resampling.nearest)
...
def main(
    ...
    for yr in years:
        ...
        # ── CDL processing ────────────────────────────────────────────────────
        if skip_cdl:
            log.info("  --skip-cdl: skipping CDL processing for year %s", yr)
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-cdl", action="store_true",
                        help="Skip CDL processing; process S2 only")
